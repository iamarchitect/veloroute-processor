...
public class SqlQueryGenerator {
	 ...
	 * @param value
	 * @param dataType
	 *            data type of the field (Specify it as "text" for string types
	 *            , "num" for numerical types)
	 * @return
	 */
	public boolean addField(String fieldName, String value, String dataType) {
		...
		if (dataType.equals("text")) {
			...
			boolean isWildchar = (StringUtilities
					.indexOfFirstContainedCharacter(value, "'") != -1);
			value = getInsertValueLeftChar(isWildchar) + value
					+ getInsertValueRightChar(isWildchar);
		}
	 ...
	 * @param aliasName
	 *            Alias name for the select field.
	 * @return
	 */
...
/**
	 ...
	 * @param val
	 *            Value for the field to be compared.
	 * @return
	 */
	...
	public boolean addWhereSubquery(String fieldName, String subQuery) {
		...
		QWhereField field = new QWhereField(fieldName, subQuery, "=", "AND",
				true);
	...
	public boolean addJoinField(String leftFielield, String rightField,
			String op) {
		...
		QWhereField field = new QWhereField(leftFielield, rightField, op,
				"AND", false);
	...
	public boolean addJoinField(String leftField, String rightField) {
		...
		QWhereField field = new QWhereField(leftField, rightField, "=", "AND",
				false);
	...
	public String getSelectQuery() {
		...
		String sField;
		boolean bFlag = false;
		Iterator<QSelectField> qSelectFieldListItr = QSelectFieldList
				.iterator();
		...
		while (qWhereFieldListItr.hasNext()) {
			...
			sQuery += whereField.fieldName + " " + whereField.operand + " "
					+ getWhereFieldLeftSeperatorChar(whereField.isSubQuery)
					+ whereField.value
					+ getWhereFieldRightSeperatorChar(whereField.isSubQuery);
			bFlag = true;
		}
		...
		while (qJoinFieldListItr.hasNext()) {
			...
			sQuery += joinField.fieldName + " " + joinField.operand
					+ joinField.value;
			bFlag = true;
		}
	...
	protected class QWhereField {
		...
		public QWhereField(String fieldName, String value, String operand,
				String concat, boolean isSubQuery) {
			this.fieldName = fieldName;
			this.value = value;
